/**
 * ReactComponentLoader - Sistema robusto de carregamento de componentes React
 * Corrige problemas de inicialização e garante carregamento adequado
 */
window.ReactComponentLoader = {
    ...
    /**
     * Inicializar o sistema de carregamento
     */
    async init() {
        ...
        this.log('🚀 Inicializando ReactComponentLoader...', 'info');
        ...
        try {
            ...
            this.log('✅ ReactComponentLoader inicializado com sucesso', 'success');
    ...
    /**
     * Garantir que dependências React estão carregadas
     */
    ...
    registerAvailableComponents() {
        ...
        this.log(`📦 Componentes registrados: ${components.join(', ')}`, 'info');
    },

    /**
     * Carregar componente de forma segura
     */
    async loadComponent(componentName, containerId, props = {}, options = {}) {
        ...
        this.log(`🔧 Carregando componente ${componentName}...`, 'info');
        ...
        return this.performComponentLoad(componentName, containerId, props, config);
    },

    /**
     * Executar carregamento do componente
     */
    async performComponentLoad(componentName, containerId, props, config) {
        ...
        this.state.loadingQueue.add(componentName);
        ...
        try {
            ...
            // Tentar carregar o componente com retry
            const success = await this.attemptComponentLoadWithRetry(
                componentName, 
                container, 
                props, 
                config
            ...
            this.state.loadingQueue.delete(componentName);
        }
    },

    /**
     * Tentar carregar componente com retry
     */
    async attemptComponentLoadWithRetry(componentName, container, props, config) {
        let lastError = null;
    ...
    log(message, level = 'info') {
        ...
        const prefix = `[ReactComponentLoader ${timestamp}]`;
    ...
    diagnose() {
        const diagnosis = {
            ...
            componentsLoading: Array.from(this.state.loadingQueue),
        ...
        console.group('🔍 Diagnóstico ReactComponentLoader');
        ...
        if (diagnosis.componentsLoading.length > 0) {
            console.info('🔄 Componentes carregando:', diagnosis.componentsLoading);
        }
    ...
    healthCheck() {
        ...
        if (!this.state.initialized) {
            health.issues.push('Sistema não inicializado');
            health.recommendations.push('Executar ReactComponentLoader.init()');
        }

        // Verificar dependências
        if (!this.state.dependenciesLoaded) {
            health.issues.push('Dependências React não carregadas');
            health.recommendations.push('Verificar carregamento de React e ReactDOM');
        }

        // Verificar componentes com falha
        if (this.state.failedComponents.size > 0) {
            health.issues.push(`${this.state.failedComponents.size} componente(s) com falha`);
            health.recommendations.push('Verificar console para erros específicos');
        }
...
window.loadReactComponent = function(componentName, containerId, props, options) {
    return window.ReactComponentLoader.loadComponent(componentName, containerId, props, options);
};

// Auto-inicialização quando DOM estiver pronto
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        if (window.ReactComponentLoader.config.autoInit) {
            window.ReactComponentLoader.init().catch(error => {
                console.error('Falha na inicialização automática do ReactComponentLoader:', error);
            });
        }
    });
} else {
